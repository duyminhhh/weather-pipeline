...
def crawl() -> str:
    ...
    # Lưu 2 file:
    # - weather_latest.json : tên cố định, dùng cho Databricks notebook
    # - weather_YYYYMMDD_HHMMSS.json : có timestamp, để archive lịch sử
    ts        = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    path_ts   = os.path.join(OUTPUT_DIR, f"weather_{ts}.json")
    path_fixed = os.path.join(OUTPUT_DIR, "weather_latest.json")

    for path in [path_ts, path_fixed]:
        with open(path, "w", encoding="utf-8") as f:
            json.dump(results, f, ensure_ascii=False, indent=2)

    print(f"\n✅ Saved {len(results)}/{len(CITIES)} cities")
    print(f"   → {path_ts}")
    print(f"   → {path_fixed}  (upload file này lên Databricks)")
    return path_fixed
